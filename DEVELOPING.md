...
## Testing the Notebook Extras

The notebook extras (`explore_dataframe`, `%dpip`, `%%sparksql`) are regular
`install_requires` dependencies, so `pip install .` is enough and there is no
"without magic support" configuration to test. The tests in
